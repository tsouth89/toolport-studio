...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@toolport-studio/ssh/auth/SshPasswordPrompt",
) {
...
# Invoked by ssh via SSH_ASKPASS when Toolport Studio re-runs ssh with a cached password
# from the renderer's in-app prompt. We never expose a native dialog here - if
# TOOLPORT_STUDIO_SSH_AUTH_SECRET is missing, that's a caller bug and we fail loudly.
if [ "\${TOOLPORT_STUDIO_SSH_AUTH_SECRET+x}" = "x" ]; then
  printf "%s\\n" "$TOOLPORT_STUDIO_SSH_AUTH_SECRET"
  exit 0
fi
printf 'Toolport Studio ssh-askpass invoked without TOOLPORT_STUDIO_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Toolport Studio re-runs\r
# ssh with a cached password from the renderer's in-app prompt. We never expose\r
# a native dialog here - if TOOLPORT_STUDIO_SSH_AUTH_SECRET is missing, that's a caller bug\r
# and we fail loudly.\r
if ($null -ne $env:TOOLPORT_STUDIO_SSH_AUTH_SECRET) {\r
  [Console]::Out.WriteLine($env:TOOLPORT_STUDIO_SSH_AUTH_SECRET)\r
  exit 0\r
}\r
[Console]::Error.WriteLine("Toolport Studio ssh-askpass invoked without TOOLPORT_STUDIO_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
> {
  ...
  return {
    ...
    ...(input.authSecret === undefined
      ? {}
      : { TOOLPORT_STUDIO_SSH_AUTH_SECRET: input.authSecret ?? "" }),
