...
} from "@toolport-studio/shared/httpReadiness";
import * as NetService from "@toolport-studio/shared/Net";
import { extractJsonObject, fromLenientJson } from "@toolport-studio/shared/schemaJson";
import { satisfiesSemverRange } from "@toolport-studio/shared/semver";
...
remote_node_satisfies_engine() {
  TOOLPORT_STUDIO_NODE_ENGINE_RANGE=@@TOOLPORT_STUDIO_NODE_ENGINE_RANGE@@
  if [ -z "$TOOLPORT_STUDIO_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$TOOLPORT_STUDIO_NODE_ENGINE_RANGE" <<'NODE'
@@TOOLPORT_STUDIO_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for TOOLPORT_STUDIO_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$TOOLPORT_STUDIO_NODE_BIN/node" ]; then
        PATH="$TOOLPORT_STUDIO_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@TOOLPORT_STUDIO_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
TOOLPORT_STUDIO_NODE_SCRIPT_PATH=@@TOOLPORT_STUDIO_NODE_SCRIPT_PATH@@
if [ -n "$TOOLPORT_STUDIO_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$TOOLPORT_STUDIO_NODE_SCRIPT_PATH" "$@"
fi
if command -v t3 >/dev/null 2>&1; then
  exec t3 "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@TOOLPORT_STUDIO_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@TOOLPORT_STUDIO_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the t3 CLI and could not install @@TOOLPORT_STUDIO_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
pick_port() {
  node - "$PORT_FILE" "@@TOOLPORT_STUDIO_DEFAULT_REMOTE_PORT@@" "@@TOOLPORT_STUDIO_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@TOOLPORT_STUDIO_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@TOOLPORT_STUDIO_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@TOOLPORT_STUDIO_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@TOOLPORT_STUDIO_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@TOOLPORT_STUDIO_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@TOOLPORT_STUDIO_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env TOOLPORT_STUDIO_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@TOOLPORT_STUDIO_READY_TIMEOUT_MS@@"; then
...
STATE_DIR="$HOME/.t3/ssh-launch/@@TOOLPORT_STUDIO_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.t3"
RUNNER_FILE="$STATE_DIR/run-t3.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  ...
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      TOOLPORT_STUDIO_PACKAGE_SPEC: packageSpec,
      TOOLPORT_STUDIO_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      TOOLPORT_STUDIO_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}
...
export function buildRemoteNodeEnvScript(input?: RemoteT3RunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      TOOLPORT_STUDIO_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      TOOLPORT_STUDIO_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(
...
export function buildRemoteLaunchScript(input?: RemoteT3RunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    TOOLPORT_STUDIO_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    TOOLPORT_STUDIO_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteT3RunnerScript(input)),
    TOOLPORT_STUDIO_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    TOOLPORT_STUDIO_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    TOOLPORT_STUDIO_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    TOOLPORT_STUDIO_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    TOOLPORT_STUDIO_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    TOOLPORT_STUDIO_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    TOOLPORT_STUDIO_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}
...
export function buildRemotePairingScript(
  ...
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    TOOLPORT_STUDIO_STATE_KEY: remoteStateKey(target),
    TOOLPORT_STUDIO_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteT3RunnerScript(input)),
  });
}
...
>()("@toolport-studio/ssh/tunnel/SshEnvironmentManager") {
