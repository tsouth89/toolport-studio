...
/**
 * Matches the image ceiling. A file attachment never enters the prompt as
 * content — only its path does — so the real cost is disk plus one upload, not
 * context.
 */
...
/**
 * A non-image attachment, handed to the agent as a path to read rather than as
 * inlined content. Images are inlined because a model reads them directly;
 * everything else is a file the agent's own Read tool is better at than we are,
 * and that keeps one code path working for every provider instead of needing a
 * native document format per CLI.
 *
 * `mimeType` is recorded for display only. Nothing dispatches on it, so an
 * unrecognised or absent browser mime type does not block the attachment.
 */
...
export const ThreadTurnQueueCommand = Schema.Struct({
  ...
  commandId: CommandId,
  threadId: ThreadId,
  message: OrchestrationQueuedTurn.fields.message,
  modelSelection: Schema.optional(ModelSelection),
  titleSeed: Schema.optional(TrimmedNonEmptyString),
  runtimeMode: RuntimeMode,
  interactionMode: ProviderInteractionMode,
...
const ThreadTurnQueueDiscardCommand = Schema.Struct({
  type: Schema.Literal("thread.turn.queue.discard"),
  commandId: CommandId,
  threadId: ThreadId,
...
const ThreadTurnQueueFlushCommand = Schema.Struct({
  type: Schema.Literal("thread.turn.queue.flush"),
  commandId: CommandId,
  threadId: ThreadId,
  messageId: MessageId,
  createdAt: IsoDateTime,
...
const DispatchableClientOrchestrationCommand = Schema.Union([
  ...
  ThreadTurnQueueDiscardCommand,
  ThreadTurnQueueFlushCommand,
...
export const ThreadTurnQueueDiscardedPayload = Schema.Struct({
  threadId: ThreadId,
  messageId: Schema.optional(MessageId),
...
export const OrchestrationEvent = Schema.Union([
  ...
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.turn-queue-discarded"),
    payload: ThreadTurnQueueDiscardedPayload,
  }),
  Schema.Struct({
    ...EventBaseFields,
