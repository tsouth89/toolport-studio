...
This guide is for people who want to use more than one Codex account in Toolport Studio.
...
In Toolport Studio Settings, name it something obvious:
...
In Toolport Studio Settings, add another Codex provider:
...
Toolport Studio shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Toolport Studio treats it
