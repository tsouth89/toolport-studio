...
describe("brand-assets", () => {
  ...
  it("uses the Studio identity consistently across release channels", () => {
    expect(BRAND_ASSET_PATHS.developmentDesktopIconPng).toBe(
      BRAND_ASSET_PATHS.productionMacIconPng,
    );
    expect(BRAND_ASSET_PATHS.nightlyMacIconPng).toBe(BRAND_ASSET_PATHS.productionMacIconPng);
    expect(BRAND_ASSET_PATHS.nightlyWindowsIconIco).toBe(
      BRAND_ASSET_PATHS.productionWindowsIconIco,
    );
    expect(BRAND_ASSET_PATHS.productionMacIconPng).toMatch(
      /^assets\/studio\/generated\/toolport-studio-/,
    );
  });
});
