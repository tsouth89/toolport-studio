...
describe("resolveCodexLaunchArgs", () => {
  it("uses TOOLPORT_STUDIO_CODEX_LAUNCH_ARGS before configured settings", () => {
    NodeAssert.equal(
      resolveCodexLaunchArgs(" --strict-config ", {
        TOOLPORT_STUDIO_CODEX_LAUNCH_ARGS: "--enable foo",
      }),
      "--enable foo",
    );
  });

  it("uses configured settings when TOOLPORT_STUDIO_CODEX_LAUNCH_ARGS is empty", () => {
    NodeAssert.equal(
      resolveCodexLaunchArgs(" --strict-config ", { TOOLPORT_STUDIO_CODEX_LAUNCH_ARGS: "   " }),
      "--strict-config",
    );
  });

  it("ignores whitespace-only environment values", () => {
    NodeAssert.equal(resolveCodexLaunchArgs("", { TOOLPORT_STUDIO_CODEX_LAUNCH_ARGS: "   " }), "");
  });
});
