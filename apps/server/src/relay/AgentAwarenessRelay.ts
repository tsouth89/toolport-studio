...
import { projectThreadAwareness } from "@toolport-studio/shared/agentAwareness";
      ...
      switch (startupState) {
        case "waiting-for-link":
          yield* Effect.logInfo(
            "agent activity publishing standby; waiting for Toolport Connect link reconciliation",
          );
          break;
        case "disabled":
          yield* Effect.logInfo(
            "agent activity publishing disabled by Toolport Connect configuration",
          );
          break;
        case "enabled":
