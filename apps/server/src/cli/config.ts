...
import { parsePersistedServerObservabilitySettings } from "@toolport-studio/shared/serverSettings";
import { DesktopBackendBootstrap, PortSchema } from "@toolport-studio/contracts";
...
export const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to TOOLPORT_STUDIO_LOG_WS_EVENTS).",
  ),
...
const EnvServerConfig = Config.all({
  logLevel: Config.logLevel("TOOLPORT_STUDIO_LOG_LEVEL").pipe(Config.withDefault("Info")),
  traceMinLevel: Config.logLevel("TOOLPORT_STUDIO_TRACE_MIN_LEVEL").pipe(
    Config.withDefault("Info"),
  ),
  traceTimingEnabled: Config.boolean("TOOLPORT_STUDIO_TRACE_TIMING_ENABLED").pipe(
    Config.withDefault(true),
  ),
  traceFile: Config.string("TOOLPORT_STUDIO_TRACE_FILE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  traceMaxBytes: Config.int("TOOLPORT_STUDIO_TRACE_MAX_BYTES").pipe(
    Config.withDefault(10 * 1024 * 1024),
  ),
  traceMaxFiles: Config.int("TOOLPORT_STUDIO_TRACE_MAX_FILES").pipe(Config.withDefault(10)),
  traceBatchWindowMs: Config.int("TOOLPORT_STUDIO_TRACE_BATCH_WINDOW_MS").pipe(
    Config.withDefault(200),
  ),
  otlpTracesUrl: Config.string("TOOLPORT_STUDIO_OTLP_TRACES_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpMetricsUrl: Config.string("TOOLPORT_STUDIO_OTLP_METRICS_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  ...
  otlpServiceName: Config.string("TOOLPORT_STUDIO_OTLP_SERVICE_NAME").pipe(
    Config.withDefault("t3-server"),
  ),
  mode: Config.schema(ServerConfig.RuntimeMode, "TOOLPORT_STUDIO_MODE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  port: Config.port("TOOLPORT_STUDIO_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("TOOLPORT_STUDIO_HOST").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  t3Home: Config.string("TOOLPORT_STUDIO_HOME").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("TOOLPORT_STUDIO_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  bootstrapFd: Config.int("TOOLPORT_STUDIO_BOOTSTRAP_FD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean(
    "TOOLPORT_STUDIO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
  ).pipe(Config.option, Config.map(Option.getOrUndefined)),
  logWebSocketEvents: Config.boolean("TOOLPORT_STUDIO_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  tailscaleServeEnabled: Config.boolean("TOOLPORT_STUDIO_TAILSCALE_SERVE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  tailscaleServePort: Config.port("TOOLPORT_STUDIO_TAILSCALE_SERVE_PORT").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
