...
export function formatServiceStatus(
  ...
  if (!status.supported) {
    return "Toolport Studio service\n  Status: unavailable on this machine\n  Supported on: Linux with systemd";
  }
  if (!status.installed) {
    return "Toolport Studio service\n  Status: not installed\n  Next: Run `t3 service install`.";
  }
  return [
    "Toolport Studio service",
...
const serviceInstallCommand = Command.make("install", projectLocationFlags).pipe(
  Command.withDescription("Install Toolport Studio as a background service for this user."),
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `Toolport Studio service is already installed with t3@${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} Toolport Studio service with t3@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
  ...
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        const result = yield* reconcileService();
        if (!result.changed) {
          yield* Console.log(`Toolport Studio service is already using t3@${packageJson.version}.`);
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} Toolport Studio service with t3@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the Toolport Studio background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed
            ? "Removed the Toolport Studio service."
            : "Toolport Studio service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the Toolport Studio background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log(
      "Toolport Studio is already set up to run in the background on this machine.",
    );
    return true;
  }
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ? "The installed Toolport Studio service needs an update or repair. Update it now?"
        : "Run Toolport Studio in the background whenever this machine boots? " +
          "It stays reachable through Toolport Connect even after you log out.",
      initial: true,
    }),
  );
  if (!wanted) {
...
export const serviceCommand = Command.make("service").pipe(
  Command.withDescription("Manage the Toolport Studio background service."),
