...
declare const __TOOLPORT_STUDIO_BUILD_RELAY_URL__: string | undefined;
declare const __TOOLPORT_STUDIO_BUILD_CLERK_PUBLISHABLE_KEY__: string | undefined;
declare const __TOOLPORT_STUDIO_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: string | undefined;
declare const __TOOLPORT_STUDIO_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: string | undefined;
declare const __TOOLPORT_STUDIO_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: string | undefined;
declare const __TOOLPORT_STUDIO_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: string | undefined;
...
export const buildTimeRelayUrl =
  typeof __TOOLPORT_STUDIO_BUILD_RELAY_URL__ === "undefined"
    ? ""
    : (normalizeSecureRelayUrl(__TOOLPORT_STUDIO_BUILD_RELAY_URL__) ?? "");
...
export const buildTimeClerkCliOAuthClientId = readBuildTimeValue(
  typeof __TOOLPORT_STUDIO_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__ === "undefined"
    ? undefined
    : __TOOLPORT_STUDIO_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__,
);
export const buildTimeRelayClientTracing = {
  tracesUrl: readBuildTimeValue(
    typeof __TOOLPORT_STUDIO_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__ === "undefined"
      ? undefined
      : __TOOLPORT_STUDIO_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__,
  ),
  tracesDataset: readBuildTimeValue(
    typeof __TOOLPORT_STUDIO_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__ === "undefined"
      ? undefined
      : __TOOLPORT_STUDIO_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__,
  ),
  tracesToken: readBuildTimeValue(
    typeof __TOOLPORT_STUDIO_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__ === "undefined"
      ? undefined
      : __TOOLPORT_STUDIO_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__,
  ),
} as const;
...
export function resolveRelayClientTracingConfig(
  ...
  const tracesUrl = env.TOOLPORT_STUDIO_RELAY_CLIENT_OTLP_TRACES_URL?.trim() || fallback.tracesUrl;
  const tracesDataset =
    env.TOOLPORT_STUDIO_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() || fallback.tracesDataset;
  const tracesToken =
    env.TOOLPORT_STUDIO_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() || fallback.tracesToken;
...
export function makeRelayUrlConfig(fallback = buildTimeRelayUrl) {
  const runtimeConfig = Config.nonEmptyString("TOOLPORT_STUDIO_RELAY_URL");
...
/**
 * Hosted app origin used for out-of-band OAuth on headless machines. There is
 * deliberately no built-in default: the CLI must never send a user's connect
 * request to a hosted deployment this build does not own, so Connect stays off
 * until a deployment configures its own origin.
 */
export const hostedAppUrlConfig = makePublicValueConfig("TOOLPORT_STUDIO_HOSTED_APP_URL", "").pipe(
...
export const hasCloudPublicConfig = Boolean(
  (normalizeSecureRelayUrl(process.env.TOOLPORT_STUDIO_RELAY_URL ?? "") ?? buildTimeRelayUrl) &&
  (process.env.TOOLPORT_STUDIO_CLERK_PUBLISHABLE_KEY?.trim() || buildTimeClerkPublishableKey) &&
  (process.env.TOOLPORT_STUDIO_CLERK_CLI_OAUTH_CLIENT_ID?.trim() || buildTimeClerkCliOAuthClientId),
);
