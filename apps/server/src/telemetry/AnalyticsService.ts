 ...
 */
import { HostProcessArchitecture, HostProcessPlatform } from "@toolport-studio/shared/hostProcess";
...
const TelemetryEnvConfig = Config.all({
  posthogKey: Config.string("TOOLPORT_STUDIO_POSTHOG_KEY").pipe(
    Config.withDefault("phc_XOWci4oZP4VvLiEyrFqkFjP4CZn55mjYYBMREK5Wd6m"),
  ),
  posthogHost: Config.string("TOOLPORT_STUDIO_POSTHOG_HOST").pipe(
    Config.withDefault("https://us.i.posthog.com"),
  ),
  enabled: Config.boolean("TOOLPORT_STUDIO_TELEMETRY_ENABLED").pipe(Config.withDefault(true)),
  flushBatchSize: Config.number("TOOLPORT_STUDIO_TELEMETRY_FLUSH_BATCH_SIZE").pipe(
    Config.withDefault(20),
  ),
  maxBufferedEvents: Config.number("TOOLPORT_STUDIO_TELEMETRY_MAX_BUFFERED_EVENTS").pipe(
