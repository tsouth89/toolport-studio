...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @toolport-studio/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --filter '@toolport-studio/scripts...' --filter '@toolport-studio/web...'",
  routes: [
    {
