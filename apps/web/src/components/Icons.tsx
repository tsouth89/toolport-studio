...
/**
 * DeepSeek's whale mark. Keeps its brand blue in both themes rather than
 * following `currentColor`, matching how the other vendor logos render.
 */
export const DeepSeekIcon: Icon = ({ className, ...props }) => (
  <svg {...props} viewBox="0 0 512 509.64" fill="none" className={cn(className)}>
    <path
      fill="#4D6BFE"
      fillRule="nonzero"
...
/**
 * Generic mark for bring-your-own-key providers, used when a preset has no
 * logo of its own. Deliberately not any vendor's logo: one driver serves
 * many providers, and borrowing a single vendor's identity for all of them
 * would be worse than a neutral key.
 */
