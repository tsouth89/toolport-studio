...
import {
  ...
  effectiveSnoozed,
  threadWokeAt,
} from "@toolport-studio/client-runtime/state/thread-settled";
...
import type { ScopedThreadRef, SidebarProjectGroupingMode } from "@toolport-studio/contracts";
...
export default function Sidebar() {
  ...
  const { settleThread, unsettleThread, snoozeThread, unsnoozeThread, deleteThread } =
    useThreadActions();
  // Studio servers do not advertise pinning yet, so these compatibility
  // callbacks remain unreachable until the protocol slice lands.
  const pinThread = settleThread;
  const unpinThread = unsettleThread;
  const reorderPinnedThread = useCallback(
    (target: ScopedThreadRef, _orderKey: string) => unsettleThread(target),
    [unsettleThread],
  );
  ...
  const projectGroups = useMemo(
    () =>
      sortLogicalProjectsForSidebar(
        ...
        threads.filter((thread) => thread.projectId !== null) as ReadonlyArray<
          EnvironmentThreadShell & { projectId: NonNullable<EnvironmentThreadShell["projectId"]> }
        >,
        sidebarProjectSortOrder,
      ),
                      ...
                      <SidebarThreadRow
                        ...
                        snoozeWakeLabelText={
                          section === "snoozed" && thread.snoozedUntil != null
                            ? snoozeWakeLabel(thread.snoozedUntil, new Date(snoozeNow))
                            : null
                        }
