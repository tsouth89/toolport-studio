...
} from "@toolport-studio/client-runtime/platform";
import {
...
import { bootstrapRemoteBearerSession } from "@toolport-studio/client-runtime/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@toolport-studio/client-runtime/environment";
import {
  managedRelayAccountChanges,
  managedRelaySessionAtom,
} from "@toolport-studio/client-runtime/relay";
import { EnvironmentRpcRequestObserver } from "@toolport-studio/client-runtime/rpc";
import {
...
function clientMetadata() {
  ...
  return {
    label: desktop ? "Toolport Studio Desktop" : "Toolport Studio Web",
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Toolport Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Toolport Connect session is unavailable.",
          });
        }
        return token;
      }),
