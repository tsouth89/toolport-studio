...
import { findErrorTraceId } from "@toolport-studio/client-runtime/errors";
import {
...
/**
 * Drives the primary environment's Toolport Connect link. Toolport Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Toolport Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[t3-connect] Could not update Toolport Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Toolport Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Toolport Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Toolport Connect before enabling this."));
        return false;
      }
