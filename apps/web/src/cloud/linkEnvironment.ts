...
import { request, runStream } from "@toolport-studio/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@toolport-studio/client-runtime/rpc";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Toolport Studio cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Toolport Studio cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "TOOLPORT_STUDIO_RELAY_URL is not configured.",
      });
    }
...
// independently of Toolport Connect while clients reach the environment out of band.
