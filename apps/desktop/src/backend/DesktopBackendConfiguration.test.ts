    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          TOOLPORT_STUDIO_PORT: "9999",
          TOOLPORT_STUDIO_MODE: "desktop",
          TOOLPORT_STUDIO_DESKTOP_LAN_HOST: "192.168.1.50",
  ...
  it.effect("resolvePrimary produces a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.TOOLPORT_STUDIO_PORT);
        assert.isUndefined(first.env.TOOLPORT_STUDIO_MODE);
        assert.isUndefined(first.env.TOOLPORT_STUDIO_DESKTOP_LAN_HOST);
