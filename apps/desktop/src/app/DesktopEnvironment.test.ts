...
const defaultInput = {
  ...
  processArch: "arm64",
  appVersion: "0.0.22",
  appPath: "/Applications/Toolport Studio.app/Contents/Resources/app.asar",
  isPackaged: false,
...
describe("DesktopEnvironment", () => {
    ...
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          ...
          TOOLPORT_STUDIO_COMMIT_HASH: " 0123456789abcdef ",
          TOOLPORT_STUDIO_PORT: "4949",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
          TOOLPORT_STUDIO_DEV_REMOTE_T3_SERVER_ENTRY_PATH: " /remote/server.mjs ",
          TOOLPORT_STUDIO_OTLP_TRACES_URL: " http://127.0.0.1:4318/v1/traces ",
          TOOLPORT_STUDIO_OTLP_EXPORT_INTERVAL_MS: "2500",
        },
      );
      ...
      assert.equal(environment.appUserModelId, "studio.toolport.desktop.dev");
      assert.equal(environment.linuxWmClass, "toolport-studio-dev");
      assert.deepEqual(
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.toolport-studio/dev");
      assert.equal(production.stateDir, "/Users/alice/.toolport-studio/userdata");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          TOOLPORT_STUDIO_DESKTOP_APP_USER_MODEL_ID: " com.t3tools.t3code.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );
