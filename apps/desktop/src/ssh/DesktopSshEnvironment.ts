...
import * as SshAuth from "@toolport-studio/ssh/auth";
import { discoverSshHosts } from "@toolport-studio/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@toolport-studio/ssh/errors";
import * as SshTunnel from "@toolport-studio/ssh/tunnel";
...
>()("@toolport-studio/desktop/ssh/DesktopSshEnvironment") {}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Toolport Studio window is not available for SSH authentication.";
      break;
