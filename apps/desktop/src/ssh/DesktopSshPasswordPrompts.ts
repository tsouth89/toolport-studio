import type { DesktopSshPasswordPromptRequest } from "@toolport-studio/contracts";
import { DesktopSshPasswordPromptResolutionInputSchema } from "@toolport-studio/contracts";
import type { SshPasswordRequest } from "@toolport-studio/ssh/auth";
...
export class DesktopSshPromptWindowUnavailableError extends Schema.TaggedErrorClass<DesktopSshPromptWindowUnavailableError>()(
  ...
  override get message(): string {
    ...
    return `Toolport Studio window is unavailable during ${this.stage} for SSH authentication to ${this.destination} (request: ${request}).`;
  }
}
...
>()("@toolport-studio/desktop/ssh/DesktopSshPasswordPrompts") {}
