...
describe("ElectronProtocol", () => {
  ...
  it.effect("proxies the stable renderer origin to the current app server", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const response = yield* Effect.promise(() =>
            handler!(
              new Request("toolport-studio-dev://app/api/health?verbose=1", {
                headers: {
                  accept: "application/json",
                  origin: "toolport-studio-dev://app",
                  referer: "toolport-studio-dev://app/",
          ...
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "img-src 'self' toolport-studio-dev: blob: data: http: https:",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "font-src 'self' toolport-studio-dev: data:",
          );
        }),
      );
      ...
      assert.deepEqual(unhandleMock.mock.calls, [["toolport-studio-dev"]]);
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          const protocol = yield* ElectronProtocol.ElectronProtocol;
          yield* protocol.registerDesktopProtocol({
            scheme: "toolport-studio",
            targetOrigin: new URL("http://127.0.0.1:3773/"),
            backendOrigin: new URL("http://127.0.0.1:3773/"),
            clerkFrontendApiHostname: undefined,
          });
          return yield* Effect.promise(() => handler!(new Request("toolport-studio://other/")));
        }),
      );
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("toolport-studio-dev://app/")));
        }),
      );
  ...
  it.effect("preserves protocol registration failures", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.scheme, "toolport-studio-dev");
      assert.strictEqual(error.cause, cause);
      assert.equal(
        error.message,
        'Failed to register Electron protocol scheme "toolport-studio-dev".',
      );
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
  ...
  it.effect("preserves protocol unregistration failures", () =>
    Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(error.scheme, "toolport-studio");
        assert.strictEqual(error.cause, cause);
        assert.equal(
          error.message,
          'Failed to unregister Electron protocol scheme "toolport-studio".',
        );
      }
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
  ...
  it("keeps executable sources host-restricted while allowing runtime network resources", () => {
    ...
    assert.deepEqual(directives["font-src"], ["'self'", "toolport-studio:", "data:"]);
  });
});
