...
function makeFakeBrowserWindow() {
  ...
  const webContents = {
    copyImageAt: vi.fn(),
    getURL: vi.fn(() => "toolport-studio-dev://app/"),
  ...
  it("recognizes only same-origin renderer navigations", () => {
    assert.isTrue(
      DesktopWindow.isSameOriginRendererNavigation({
        applicationUrl: "toolport-studio://app/",
        navigationUrl: "toolport-studio://app/settings/connections",
      }),
    );
    assert.isFalse(
      DesktopWindow.isSameOriginRendererNavigation({
  ...
  it("retries only transient failures for the development renderer", () => {
    assert.isTrue(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
        applicationUrl: "toolport-studio-dev://app/",
        errorCode: -102,
        isMainFrame: true,
        validatedUrl: "toolport-studio-dev://app/",
      }),
    );
    assert.isFalse(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
