...
import { buildRemoteNodeEnvScript } from "@toolport-studio/ssh/tunnel";
...
>()("@toolport-studio/desktop/wsl/DesktopWslEnvironment") {}
...
export const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_PREBUILD_MISSING_EXIT_CODE
    ? "WSL support is missing from this Toolport Studio build: the packaged Linux node-pty binary was not included. Rebuild the Windows artifact with `--wsl-prebuild <path-to-linux-pty.node>` or install a build that includes WSL support."
    : null;
