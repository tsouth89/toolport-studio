// This file mostly exists because we want dev mode to say "Toolport Studio (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Toolport Studio (Dev)" : "Toolport Studio (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `studio.toolport.desktop.dev.${devBundleIdSuffix || "local"}`
  : "studio.toolport.desktop";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["toolport-studio-dev"] : ["toolport-studio"];
...
const developmentMacIconPngPath = NodePath.join(
  ...
  "studio",
  "generated",
  "toolport-studio-desktop-1024.png",
);
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["TOOLPORT_STUDIO_PORT", environment.TOOLPORT_STUDIO_PORT],
    ["TOOLPORT_STUDIO_HOME", environment.TOOLPORT_STUDIO_HOME],
    ["TOOLPORT_STUDIO_COMMIT_HASH", environment.TOOLPORT_STUDIO_COMMIT_HASH],
    ["TOOLPORT_STUDIO_OTLP_TRACES_URL", environment.TOOLPORT_STUDIO_OTLP_TRACES_URL],
    [
      "TOOLPORT_STUDIO_OTLP_EXPORT_INTERVAL_MS",
      environment.TOOLPORT_STUDIO_OTLP_EXPORT_INTERVAL_MS,
    ],
    ["TOOLPORT_STUDIO_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Toolport Studio name.
